...
const CatPage = () => {
  ...
  const fetchProducts = () => {
    ...
    api.get(`catproducts/${queryString}`).then((res) => setProducts(res.data));
  };

  useEffect(() => {
    api.get("catbanners/").then((res) => setBanners(res.data));
  }, []);

  useEffect(() => {
    api.get("catproducts/filters/").then((res) => {
    ...
    <div className="flex flex-col lg:flex-row p-4 lg:p-6 gap-6">
      ...
      <div className="w-full lg:w-3/4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3 sm:gap-0">
          <h2 className="text-xl font-bold">Cat Products</h2>
          <select
